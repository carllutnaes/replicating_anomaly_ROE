...
roe_vw_all_p_r <- NULL#list(,n_portfolios)
roe_vw_all_p_r_str_low <- sprintf("%s_Low", selected_variable_str)
roe_vw_all_p_r_str_high <- sprintf("%s_High", selected_variable_str)

roe_vw_all_p_r_names <-
  c(roe_vw_all_p_r_str_low,
    "P2",
    "P3",
    "P4",
    roe_vw_all_p_r_str_high)
...
for (y in 1:length(s.year)) {
  ...
  for (p in 1:n_portfolios) {
    ...
    colnames(r_y) <- roe_vw_all_p_r_names[p]
    temp_x <- cbind(temp_x, r_y)
    
  }
  roe_vw_all_p_r <- rbind(roe_vw_all_p_r, temp_x)
...
hedge_portfolio <-
  roe_vw_all_p_r[, roe_vw_all_p_r_str_high] - roe_vw_all_p_r[, roe_vw_all_p_r_str_low]
colnames(hedge_portfolio) <-
  sprintf("r_%s_HML", selected_variable_str)
roe_vw_all_p_r <- cbind(roe_vw_all_p_r, hedge_portfolio)

roe_vw_all_p_r_win <- NULL
roe_vw_all_p_r_trim <- NULL

wi <- 6
roe_vw_all_p_r <- roe_vw_all_p_r[!is.na(roe_vw_all_p_r[, wi]), ]
pp <- roe_vw_all_p_r[, wi]
p_max <- IQR(pp) * 1.5 + median(pp)
p_min <- -IQR(pp) * 1.5 + median(pp)
roe_vw_all_p_r_win <-
  cbind(roe_vw_all_p_r_win, Winsorize(pp, minval = p_min, maxval = p_max))

qnt <-
  quantile(roe_vw_all_p_r[, wi],
           probs = c(.025, .975),
           na.rm = TRUE)
pp <- subset(roe_vw_all_p_r[, wi],
             roe_vw_all_p_r[, wi] >= qnt[1] &
               roe_vw_all_p_r[, wi] <= qnt[2])
roe_vw_all_p_r_trim <- cbind(roe_vw_all_p_r[index(pp), 1:5], pp)
#}

manual_date_rmv <- "1995-04-30"
sorted_dates <-
  which(!grepl(manual_date_rmv, index(roe_vw_all_p_r_trim)))
roe_vw_all_p_r_trim <- roe_vw_all_p_r_trim[sorted_dates]
